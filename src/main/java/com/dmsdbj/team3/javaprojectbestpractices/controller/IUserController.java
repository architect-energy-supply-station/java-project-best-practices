...
public interface IUserController {
    ...
    ResultBean removeUser(@ApiParam(value = "id", required = true,defaultValue = "2") @PathVariable("id") int id);

    @ApiOperation(value = "新增一个用户", notes = "请输入要新增的用户信息")
    @PostMapping("/user/save")
    ResultBean saveUser(@ApiParam(value = "user") @RequestBody User user);
