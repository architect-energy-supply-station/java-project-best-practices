...
public interface IUserController {
    ...
    User getUser(@ApiParam(value = "id", required = true,defaultValue = "1") @PathVariable("id") int id);

    @ApiOperation(value = "查询所有用户")
    @GetMapping("/user/list")
    IPage getUserList(@ApiParam(value = "page") Page page);
    ...
    String saveUser(@ApiParam(value = "user") @RequestBody User user);

    @ApiOperation(value = "根据姓名模糊查询", notes = "请输入要模糊查询的信息")
    @GetMapping(value = {"/user/getUserByLikeName/{name}"})
    List<User> getUserByLikeName(@ApiParam(value = "name", required = true,defaultValue ="xiaoli") @PathVariable String name);
}
